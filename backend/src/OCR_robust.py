...
class PatientManager:
    ...
    def normalize_name(self, name):
        ...
        name = re.sub(
            r"\b(mr|mrs|ms|dr|prof|miss|patient|name)\b\.?",
            "",
            name,
            flags=re.IGNORECASE,
        )
...
def get_flag(value, ref_range):
    ...
    try:
        # Extract ALL pure float values safely to handle OCR clumping (e.g., "11% (0.5 abs)")
        val_matches = re.findall(r"\d+(?:\.\d+)?", value)
        if not val_matches:
            return "Unknown"
        val_floats = [float(v) for v in val_matches]

        # Check explicit operators: <, >, Up to, More than
        lower_ref = ref_range.lower()
        if "<" in lower_ref or "up to" in lower_ref or "less than" in lower_ref:
            limit_match = re.search(r"\d+(?:\.\d+)?", lower_ref)
            if limit_match:
                limit = float(limit_match.group())
                if any(v <= limit for v in val_floats):
                    return "Normal"
                return "High"

        if ">" in lower_ref or "more than" in lower_ref:
            limit_match = re.search(r"\d+(?:\.\d+)?", lower_ref)
            if limit_match:
                limit = float(limit_match.group())
                if any(v >= limit for v in val_floats):
        ...
        # Check standard range X - Y or X to Y
        range_match = re.search(
            r"(\d+(?:\.\d+)?)\s*(?:-|to)\s*(\d+(?:\.\d+)?)", ref_range, re.IGNORECASE
        )
        # Fallback: if range looks like "4.0 10.0 -"
        if not range_match:
            range_match = re.search(
                r"(\d+(?:\.\d+)?)\s+(\d+(?:\.\d+)?)\s*-?\s*$", ref_range
            )
        ...
        if range_match:
            ...
            # If any of the extracted numbers falls in the target range, it's clinically normal
            if any(low <= v <= high for v in val_floats):
                return "Normal"

            # If none fall in range, default to comparing the very first number (the primary metric)
            first_val = val_floats[0]
            if first_val < low:
                return "Low"
            if first_val > high:
                return "High"
        ...
        print(f"DEBUG flag exception: {e} with val {value} and ref {ref_range}")
        pass
    return "Unknown"
...
class RobustOCR:
    def __init__(self):
        ...
        self.loinc_mappings = self.load_loinc_mappings()

        # Pre-load heavy OCR models to avoid re-loading on every request
        print("Initializing heavy OCR models...")
        self.doc_model = ocr_predictor(pretrained=True, detect_orientation=True)
        self.verifier = easyocr.Reader(["en"], gpu=False, verbose=False)
        self.name_reader = easyocr.Reader(["ar", "en"], gpu=False, verbose=False)

    def load_loinc_mappings(self):
        try:
            mapping_path = os.path.join(
                os.path.dirname(__file__), "config", "loinc_mapping.json"
            )
            with open(mapping_path, "r", encoding="utf-8") as f:
                return json.load(f).get("mappings", {})
        except Exception as e:
            print(f"Warning: Could not load LOINC mappings: {e}")
            return {}

    def load_test_mappings(self):
        try:
            mapping_path = os.path.join(
                os.path.dirname(__file__), "config", "test_mapping.json"
            )
            with open(mapping_path, "r", encoding="utf-8") as f:
                return json.load(f).get("mappings", {})
        except Exception as e:
    ...
    def get_test_code(self, ocr_name):
        ...
        matches = difflib.get_close_matches(
            norm_name, self.test_mappings.keys(), n=1, cutoff=0.7
        )
        if matches:
            best_match = matches[0]
            print(
                f"Fuzzy Match: '{ocr_name}' -> '{best_match}' ({self.test_mappings[best_match]})"
            )
    ...
    def is_mostly_symbols(self, text):
        ...
        if punct_ratio > 0.4:
            print(
                f"DEBUG: Rejected '{text}' as noise (Punct Density: {punct_ratio:.2f})"
            )
            return True
    ...
    def is_noise(self, test_name):
        ...
        if re.match(r"^[\d\W]+$", test_name):
            return True

        # 2. IDs like U2199
        if re.match(r"^[A-Z][\d-]+$", test_name, re.IGNORECASE):
            return True
    ...
    def preprocess_image(self, img, page_num):
        """
        Applies simple preprocessing and orientation fixes.
        Auto-rotates landscape images 90 degrees clockwise.
        """
        h, w = img.shape[:2]
        if w > h:
            print(
                f"DEBUG: Page {page_num} is landscape ({w}x{h}). Auto-rotating 90 degrees CW."
            )
    ...
    def find_header_row(self, pages):
        ...
        header_patterns = {
            ...
            "Unit": (r"(unit)", r"unit"),
            "Reference Range": (
                r"(refer.*range|normal.*values)",
                r"refer|range|normal",
            ),
        }
        ...
        return (
            0,
            0.15,
            {"Test Name": 0.1, "Value": 0.4, "Unit": 0.6, "Reference Range": 0.8},
        )
    ...
    def extract_patient_name(self, image_path, header_bottom, doc=None):
        ...
        searches = [
            ...
            r"(?:Patient\s*Name|Name)\s*[:\-\.]?\s*(?:\d+[\d:]*\s+)?("
            + patt_chars
            + r"?)\s+(?:ID|Ref|Date|Sex|Age|File|Lab|Coll|Auth|Print|Test|Res|Unit|Visit)",
            # 2. Arabic "Name" (Ism | Al-Ism)
            r"(?:الاسم|اسم المريض)\s*[:\-\.]?\s*("
            + patt_chars
            + r"?)\s+(?:ID|Ref|Date|Sex|Age|File|Lab|Coll|Visit)",
            ...
            for i, pattern in enumerate(searches):
                ...
                if match:
                    ...
                    if len(extracted_clean) > 2:
                        print(
                            f"Found Patient Name (EasyOCR - Pat {i}): {extracted_clean}"
                        )
            ...
            try:
                ...
                for i, pattern in enumerate(searches):
                    ...
                    if match:
                        ...
                        if len(extracted_clean) > 2:
                            print(
                                f"Found Patient Name (Doctr - Pat {i}): {extracted_clean}"
                            )
                            return extracted_clean
    ...
    def process_document(self, file_path_or_images, patient_manager=None):
        if isinstance(
            file_path_or_images, str
        ) and file_path_or_images.lower().endswith(".pdf"):
        ...
        doc = self.doc_model(DocumentFile.from_images(image_paths))
        ...
        raw_patient_name = self.extract_patient_name(
            first_page_img, header_bottom if start_page == 0 else 0.3, doc
        )
        ...
        if patient_manager:
            patient_id, normalized_name = patient_manager.get_or_create_id(
                raw_patient_name
            )
        ...
        for p_idx, page in enumerate(doc.pages):
            ...
            if words_below:
                ...
                for w in words_below[1:]:
                    if (
                        abs(w.geometry[0][1] - current_row[-1].geometry[0][1])
                        < row_threshold
                    ):
            ...
            for row in rows:
                ...
                # Check Footers
                if (
                    "signature" in row_text_full.lower()
                    or "professor" in row_text_full.lower()
                ):
                ...
                for w in row:
                    ...
                    for col_name, (c_start, c_end) in col_ranges.items():
                        if c_start <= center_x <= c_end:
                            row_cols[col_name].append(w)
                            break

                # Extract Text
                name_text = fix_spacing(
                    " ".join([w.value for w in row_cols["Test Name"]])
                )
                val_text = " ".join([w.value for w in row_cols["Value"]])
                unit_text = " ".join([w.value for w in row_cols["Unit"]])
                ref_text = " ".join([w.value for w in row_cols["Reference Range"]])

                # --- 🧠 MULTI-PASS ENSEMBLE VERIFICATION FOR VALUES ---
                val_confidences = [
                    getattr(w, "confidence", 1.0) for w in row_cols["Value"]
                ]
                avg_conf = (
                    sum(val_confidences) / len(val_confidences)
                    if val_confidences
                    else 1.0
                )

                if (
                    any(char.isdigit() for char in val_text)
                    and len(row_cols["Value"]) > 0
                    and avg_conf < 0.90
                ):
                    try:
                        xmin = min(w.geometry[0][0] for w in row_cols["Value"])
                        ymin = min(w.geometry[0][1] for w in row_cols["Value"])
                        xmax = max(w.geometry[1][0] for w in row_cols["Value"])
                        ymax = max(w.geometry[1][1] for w in row_cols["Value"])

                        img = cv2.imread(image_paths[p_idx])
                        h_img, w_img, _ = img.shape

                        x1 = max(0, int(xmin * w_img) - 5)
                        y1 = max(0, int(ymin * h_img) - 5)
                        x2 = min(w_img, int(xmax * w_img) + 5)
                        y2 = min(h_img, int(ymax * h_img) + 5)

                        crop = img[y1:y2, x1:x2]

                        # Only proceed if crop is valid
                        if crop.size > 0:
                            # Upscale 2x for secondary verification pass
                            crop_2x = cv2.resize(
                                crop, None, fx=2, fy=2, interpolation=cv2.INTER_CUBIC
                            )
                            veri_res = self.verifier.readtext(crop_2x, detail=0)
                            veri_text = " ".join(veri_res)

                            if veri_text and veri_text.replace(
                                " ", ""
                            ) != val_text.replace(" ", ""):
                                # Clean up EasyOCR outputs that might have artifacts
                                cleaned_veri = re.sub(r"[^\d\.\<E]", "", veri_text)
                                cleaned_val = re.sub(r"[^\d\.\<E]", "", val_text)

                                # If the digits actually differ
                                if (
                                    cleaned_veri
                                    and cleaned_val
                                    and cleaned_veri != cleaned_val
                                ):
                                    print(
                                        f"🥇 Ensemble Tie-Break Triggered: Doctr='{val_text}' vs EasyOCR='{veri_text}' on '{name_text}'. Trusting Verified Pass."
                                    )
                                    val_text = veri_text
                    except Exception as e:
                        print(f"Warning: OCR tie-breaker failed gracefully: {e}")
                # -----------------------------------------------------

                # --- Post-Processing for CBC Squished Columns ---
                clean_name_upper = name_text.upper()
                cbc_tests = [
                    "NEUTROPHIL",
                    "LYMPHOCYTE",
                    "MONOCYTE",
                ...
                if any(t in clean_name_upper for t in cbc_tests):
                    # In some reports, Percentage, Absolute, and Unit squeeze into the Unit or Value columns
                    combined = f"{val_text} {unit_text}".strip()

                    # 1. Extract and standardize the unit
                    unit_match = re.search(
                        r"(10\**[\^]?\d+/[uU]?[lL]|1013/[uU]?[lL])",
                        combined,
                        re.IGNORECASE,
                    )
                    found_unit = ""
                    if unit_match:
                        # Fix common OCR 1013 to 10*3 error
                        found_unit = unit_match.group(1).replace("1013", "10*3")
                        combined = combined.replace(unit_match.group(1), "").strip()

                    # 2. Extract remaining numbers
                    numbers = re.findall(r"\d+\.\d+|\d+", combined)

                    if len(numbers) >= 2:
                        # First number is usually %, second is Absolute
                        val_text = f"{numbers[0]}% ({numbers[1]} abs)"
                        unit_text = found_unit if found_unit else "10*3/uL"
                    elif len(numbers) == 1:
                        val_text = numbers[0]
                        unit_text = found_unit if found_unit else "10*3/uL"

                # --- Crop Row Image Snippet ---
                crop_path = None
                if row:
                    try:
                        ymin_row = min(w.geometry[0][1] for w in row)
                        ymax_row = max(w.geometry[1][1] for w in row)

                        img_full = cv2.imread(image_paths[p_idx])
                        if img_full is not None:
                            h_full, w_full, _ = img_full.shape
                            y_start = max(0, int(ymin_row * h_full) - 15)
                            y_end = min(h_full, int(ymax_row * h_full) + 15)

                            crop_strip = img_full[y_start:y_end, 0:w_full]

                            import uuid

                            crop_filename = f"crop_{uuid.uuid4().hex[:8]}.png"
                            crop_dir = os.path.join(OUTPUT_DIR, "processed")
                            os.makedirs(crop_dir, exist_ok=True)

                            if crop_strip.size > 0:
                                cv2.imwrite(
                                    os.path.join(crop_dir, crop_filename), crop_strip
                                )
                                crop_path = f"processed/{crop_filename}"
                    except Exception as e:
                        print(f"Warning: Failed to crop row snippet: {e}")
                ...
                entry = {
                    ...
                    "Flag": get_flag(val_text, ref_text),
                ...
                entry["Test_Code"] = (
                    norm_name if norm_name != clean_name.upper() else clean_name.upper()
                )
                ...
                # Data Validation
                is_valid_val = re.search(
                    r"\d+|negative|positive|reactive", val_text, re.I
                )
                ...
                if is_valid_val:
                    ...
                    entry["Test_Code"] = test_code if test_code else norm_name

                    if entry["Test_Code"] in self.loinc_mappings:
                        entry["LOINC_Code"] = self.loinc_mappings[entry["Test_Code"]]

                    # Value Type
                    if "%" in unit_text or "%" in val_text:
                        entry["Value_Type"] = "PERCENT"
                    elif (
                        test_code in ["NEUT", "LYMPH", "MONO", "EO", "BASO"]
                        and not "%" in unit_text
                    ):
                    ...
                    # Reliability
                    val_confidences = [w.confidence for w in row_cols["Value"]]
                    avg_conf = (
                        sum(val_confidences) / len(val_confidences)
                        if val_confidences
                        else 0.9
                    ...
                    entry["Reliability_Level"] = self.get_reliability_level(
...
if __name__ == "__main__":
    ...
    for pdf_file in pdf_files:
        ...
        if file_results:
            ...
            p_data = {"id": patient_info[0], "name": patient_info[1]}
    ...
    try:
        with pd.ExcelWriter(output_path, engine="openpyxl") as writer:
            # Sheet 1: Patients
            df_patients = pd.DataFrame(patient_registry).drop_duplicates(
                subset=["Patient_ID"]
            )
