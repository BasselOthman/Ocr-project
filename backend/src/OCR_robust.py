...
def get_flag(value, ref_range):
    ...
    try:
        ...
        if not val_matches: return "Unknown"
        ...
        if "<" in lower_ref or "up to" in lower_ref or "less than" in lower_ref:
            ...
            if limit_match:
                ...
                if any(v <= limit for v in val_floats): return "Normal"
                return "High"
        ...
        range_match = re.search(r'(\d+(?:\.\d+)?)\s*(?:-|to)\s*(\d+(?:\.\d+)?)', ref_range, re.IGNORECASE)
        # Fallback: if range looks like "4.0 10.0 -"
        if not range_match:
            range_match = re.search(r'(\d+(?:\.\d+)?)\s+(\d+(?:\.\d+)?)\s*-?\s*$', ref_range)
        ...
        if range_match:
            ...
            if first_val < low: return "Low"
            if first_val > high: return "High"
            return "Normal"
...
class RobustOCR:
    ...
    def preprocess_image(self, img, page_num):
        ...
        h, w = img.shape[:2]
        if w > h:
            print(f"DEBUG: Page {page_num} is landscape ({w}x{h}). Auto-rotating 90 degrees CW.")
    ...
    def process_document(self, file_path_or_images, patient_manager=None):
        ...
        for p_idx, page in enumerate(doc.pages):
            ...
            for row in rows:
                ...
                # Extract Text
                name_text = fix_spacing(" ".join([w.value for w in row_cols["Test Name"]]))
                ...
                if any(char.isdigit() for char in val_text) and len(row_cols["Value"]) > 0:
                    try:
                        ...
                        if crop.size > 0:
                            ...
                            if veri_text and veri_text.replace(" ", "") != val_text.replace(" ", ""):
                                ...
                                if cleaned_veri and cleaned_val and cleaned_veri != cleaned_val:
                                    print(f"🥇 Ensemble Tie-Break Triggered: Doctr='{val_text}' vs EasyOCR='{veri_text}' on '{name_text}'. Trusting Verified Pass.")
                                    val_text = veri_text
                    except Exception as e:
                ...
                cbc_tests = ["NEUTROPHIL", "LYMPHOCYTE", "MONOCYTE", "EOSINOPHIL", "BASOPHIL"]
                if any(t in clean_name_upper for t in cbc_tests):
                    ...
                    unit_match = re.search(r'(10\**[\^]?\d+/[uU]?[lL]|1013/[uU]?[lL])', combined, re.IGNORECASE)
                    found_unit = ""
                    if unit_match:
