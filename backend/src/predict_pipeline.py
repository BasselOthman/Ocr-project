...
LOAD_DIR = os.environ.get("MODEL_DIR", os.path.join(BASE_DIR, "nn_balanced_batch_saved"))
...
class PredictionPipeline:
    def __init__(self):
        ...
        try:
            ...
            self.model.load_state_dict(torch.load(os.path.join(LOAD_DIR, "model_state_dict.pth"), map_location=DEVICE))
            self.model.eval()
            self.is_loaded = True
    ...
    def predict(self, ocr_results_list, patient_demographics=None):
        ...
        if patient_demographics:
            if "gender" in patient_demographics:
                features_dict["gender"] = 1 if patient_demographics["gender"].upper() == "M" else 0
        ...
        # 8. Assemble Results against thresholds
        results = {}
